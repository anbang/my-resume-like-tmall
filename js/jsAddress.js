var addressInit = function(province, city, area, defaultProvince, defaultCity, defaultArea) {
	...
	//选择默认的省
	select(province, defaultProvince);
	changeProvince();
	province.onchange = changeProvince;

	//创建option标签集合，配合select使用；
	...
	function changeProvince() {
		//当省份改变的时候，先清一下上次选择的城市选项
		city.options.length = 0;//默认显示的城市名
		city.onchange = null;//清空函数；
	...
	function changeCity() {
		...
		//默认地区
		select(area, defaultArea);
	}

	function select(cmb, str) {
